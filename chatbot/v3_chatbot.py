...
st.set_page_config(page_title="AI Data Analyst NooB", layout="wide", initial_sidebar_state="expanded", page_icon="logo.ico")
...
############### Modified PDF Q&A Section 20-08-2025 #####################


def display_pdf_qa_section(pdf_content):
    """Interactive ChatGPT-style Q&A interface for PDF documents - WITH FEEDBACK"""
    ...
    # Clear button - with controlled rerun
    ...
    for i, msg in enumerate(st.session_state.pdf_chat_history):
        if msg['role'] == 'user':
            ...
            with st.chat_message("assistant"):
                ...
                # ✅ ADD FEEDBACK SYSTEM FOR EACH ASSISTANT MESSAGE
                if 'query' in msg:  # Only show feedback for messages that have associated queries
                    display_document_feedback_system(msg['query'], msg, f"pdf_{i}")
    ...
    with st.expander("💡 Try a suggested question", expanded=len(st.session_state.pdf_chat_history) == 0):
        ...
        for i, q in enumerate(sample_questions[:4]):
            button_key = f"sugg_q_pdf_{i}_{len(st.session_state.pdf_chat_history)}"
    ...
    if prompt := st.chat_input("Ask a question about your PDF"):
        ...
        with st.chat_message("assistant"):
            with st.spinner("🤖 Thinking..."):
                try:
                    # Process question using EXISTING pdf_content
                    answer_result = pdf_agent.answer_question(prompt.strip(), pdf_content)
                    ...
                    assistant_msg = {
                        ...
                        'confidence': confidence,
                        'method': method,
                        'query': prompt.strip()  # ✅ ADD QUERY TO ASSISTANT MESSAGE FOR FEEDBACK
                    }
                    
                    # Add page references to message if available
                    if answer_result.get('relevant_pages'):
                        pages_str = ", ".join(map(str, sorted(answer_result['relevant_pages'])))
                    ...
                    # ✅ SHOW FEEDBACK SYSTEM FOR THIS NEW RESPONSE
                    display_document_feedback_system(prompt.strip(), assistant_msg, f"pdf_new_{len(st.session_state.pdf_chat_history)}")
                    ...
                    try:
                        if (st.session_state.get('authenticated') and 
                            ...
                            save_success = st.session_state.conv_manager.save_conversation(
                                username=st.session_state.username,
                                conversation_type='pdf_qa',
                                query=prompt.strip(),
                                response=response,
                                confidence=confidence,
                                file_type='pdf',
                            ...
                            if save_success:
                                logger.info("✅ PDF Q&A conversation saved successfully")
                            else:
                                logger.error("❌ PDF Q&A conversation save returned False")
                                
                    except Exception as save_error:
                        logger.error(f"❌ CRITICAL: PDF Q&A save failed: {save_error}")
...
##################### Modified PDF Question Processing Function 20-08-2025 #####################

def _process_pdf_question(question: str, pdf_agent, pdf_content):
    """Process PDF question and add to history immediately - for suggestion buttons"""
    ...
    try:
        ...
        logger.info(f"PDF Q&A: Suggestion question processed with {confidence:.2f} confidence")
        
    except Exception as e:
        # Handle errors gracefully - UPDATE the already initialized variables
    ...
    # Save to conversation history (enhanced) - NOW variables are always defined
    try:
        if (st.session_state.get('authenticated') and 
            st.session_state.get('username') and 
            st.session_state.get('conv_manager')):
            ...
            if save_success:
                logger.info(f"✅ PDF Q&A conversation saved for {st.session_state.username}")
            else:
                logger.error(f"❌ PDF Q&A conversation save failed for {st.session_state.username}")
            
            # Track usage
            if st.session_state.get('usage_tracker'):
                st.session_state.usage_tracker.log_query(
                    username=st.session_state.username,
                    query=question.strip(),
                    response=response,
                    confidence=confidence,
                ...
                logger.info(f"✅ PDF Q&A usage tracked for {st.session_state.username}")
        else:
            logger.warning(f"⚠️ PDF Q&A save skipped - Auth: {st.session_state.get('authenticated')}, User: {st.session_state.get('username')}, ConvMgr: {st.session_state.get('conv_manager') is not None}")
            
    except Exception as save_error:
        logger.error(f"❌ Failed to save PDF Q&A conversation: {save_error}")
...
def extract_comprehensive_word_content(word_file):
    ...
    if content and content['status'] == 'success':
        ...
        return None
########################## Advanced Word Processor added on 18-08-2025 ##########################
def handle_word_file_upload(uploaded_file):
    """Handle Word file upload - Similar to PDF handler"""
    ...
    if word_content:
        # Store in session state
        st.session_state.current_word_content = word_content
        st.session_state.file_type = 'word'
        st.session_state.current_df = None
        st.session_state.current_pdf_content = None
        
        # Display summary metrics
        metadata = word_content.get('metadata', {})
        ...
        # Add Q&A interface
        st.markdown("---")
        display_word_qa_section(word_content)
        
        # Add document explorer
        # with st.expander("🔍 Document Explorer"):
        #     display_word_explorer(word_content)
        
        return True
    
    return False
########################## Modifired Word Processor added on 20-08-2025 ##########################

def display_word_qa_section(word_content):
    """Interactive Q&A interface for Word documents - WITH FEEDBACK"""

    if 'word_agent' not in st.session_state:
        st.warning("Word agent not available for Q&A")
        return

    word_agent = st.session_state['word_agent']
    ...
    col_clear, col_info = st.columns([1, 4])
    with col_clear:
        if st.button("🗑️ Clear Conversation", key="clear_word_chat"):
            st.session_state.word_chat_history = []
            st.rerun()
    
    with col_info:
        if st.session_state.word_chat_history:
            st.caption(f"💬 {len(st.session_state.word_chat_history)//2} questions asked")

    # Render existing conversation history
    for i, msg in enumerate(st.session_state.word_chat_history):
        if msg['role'] == 'user':
            with st.chat_message("user"):
                st.markdown(msg['message'])
        else:
            with st.chat_message("assistant"):
                ...
                # ✅ ADD FEEDBACK SYSTEM FOR EACH ASSISTANT MESSAGE
                if 'query' in msg:  # Only show feedback for messages that have associated queries
                    display_document_feedback_system(msg['query'], msg, f"word_{i}")

    # Suggested questions
    with st.expander("💡 Try a suggested question", expanded=len(st.session_state.word_chat_history) == 0):
        cols = st.columns(2)
        sample_questions = [
            "What is the main topic of this document?",
            "Summarize the key findings.",
        ...
        for i, q in enumerate(sample_questions[:4]):
            button_key = f"word_sugg_q_{i}_{len(st.session_state.word_chat_history)}"
            if cols[i % 2].button(q, key=button_key, use_container_width=True):
                _process_word_question(q, word_agent, word_content)
                st.rerun()

    # Chat input box
    if prompt := st.chat_input("Ask a question about your Word document"):
        # Show user message immediately
        with st.chat_message("user"):
            st.markdown(prompt)
        
        # Add to history
        st.session_state.word_chat_history.append({
            'role': 'user', 
            'message': prompt.strip()
        })
        ...
        with st.chat_message("assistant"):
            with st.spinner("🤖 Thinking..."):
                try:
                    # Process question using EXISTING word_content
                    answer_result = word_agent.answer_question(prompt.strip(), word_content)
                    ...
                    # Add section references if available
                    if answer_result.get('relevant_sections'):
                        sections_str = ", ".join(answer_result['relevant_sections'])
                        st.caption(f"*Referenced sections: {sections_str}*")
                    ...
                    # Add section references to message if available
                    if answer_result.get('relevant_sections'):
                        sections_str = ", ".join(answer_result['relevant_sections'])
                        assistant_msg['message'] += f"\n\n*Referenced sections: {sections_str}*"
                    
                    # Add to history
                    st.session_state.word_chat_history.append(assistant_msg)
                    
                    # ✅ SHOW FEEDBACK SYSTEM FOR THIS NEW RESPONSE
                    display_document_feedback_system(prompt.strip(), assistant_msg, f"word_new_{len(st.session_state.word_chat_history)}")
                    ...
                    try:
                        if (st.session_state.get('authenticated') and 
                            ...
                            if save_success:
                                logger.info("✅ Word Q&A conversation saved successfully")
                            else:
                                logger.error("❌ Word Q&A conversation save returned False")
                                
                    except Exception as save_error:
                        logger.error(f"❌ CRITICAL: Word Q&A save failed: {save_error}")
                    ...
                    logger.error(f"Word Q&A error: {e}")

########################## Modified Word Processor added on 20-08-2025 ##########################
def _process_word_question(question: str, word_agent, word_content):
    """Process Word question and add to history immediately - for suggestion buttons"""
    ...
    try:
        # Process question using EXISTING word_content
        answer_result = word_agent.answer_question(question.strip(), word_content)
        ...
        logger.info(f"Word Q&A: Suggestion question processed with {confidence:.2f} confidence")
    ...
    # Save to conversation history (enhanced)
    try:
        if (st.session_state.get('authenticated') and 
            st.session_state.get('username') and 
            st.session_state.get('conv_manager')):
            ...
            if save_success:
                logger.info(f"✅ Word Q&A conversation saved for {st.session_state.username}")
            else:
                logger.error(f"❌ Word Q&A conversation save failed for {st.session_state.username}")
            ...
            if st.session_state.get('usage_tracker'):
                ...
                logger.info(f"✅ Word Q&A usage tracked for {st.session_state.username}")
                
    except Exception as save_error:
        logger.error(f"❌ Failed to save Word Q&A conversation: {save_error}")
# ============================================================================
...
# ==================================================================================
# 9. 🎯 MAIN QUERY PROCESSING ENGINE WITH PREDICTIVE MODELING - Master Orchestrator
# ==================================================================================
...
def process_enhanced_query(df, query, conversation_history=None):
    """🎼 ORCHESTRATOR: Routes queries to specialist agents with intelligent fallbacks - FIXED VERSION"""
    ...
    base_result = {
        ...
        'method': 'orchestrated',
        'answer': '',  # Ensure answer always exists
        'confidence': 0.0  # Ensure confidence always exists
    }
    ...
    if lookup_result:
        ...
        # 🔥 IMMEDIATE SAVE AFTER SUCCESSFUL PROCESSING
        _save_conversation_immediately(query, base_result)
        return base_result
    ...
    not any(viz_word in query.lower() for viz_word in ['plot', 'chart', 'graph', 'show', 'visualize', 'display']):
        try:
            # Your existing statistical processing logic...
            filters_applied = []
            working_df = df.copy()
            ...
            # Target column detection (keeping your existing logic)
            target_column = None

            # Method 1: Look for numeric columns in context entities
            if context['entities']:
                for entity in context['entities']:
                    if entity in df.columns and pd.api.types.is_numeric_dtype(df[entity]):
                        target_column = entity
                        break

            # Method 2: If no numeric column found in entities, search for column names in query
            if not target_column:
                filter_cols = [f.split()[0] for f in filters_applied] if filters_applied else []
                
                for col in df.columns:
                    if (col.lower() in query_lower and 
                        col not in filter_cols and 
                ...
                if not target_column:
                    for col in df.columns:
                        if (any(part in col.lower() for part in query_lower.split()) and 
                            col not in filter_cols and 
                            pd.api.types.is_numeric_dtype(df[col])):
                            target_column = col
                            break

            # Method 3: Statistical keywords
            if not target_column:
                statistical_keywords = ['price', 'cost', 'amount', 'value', 'total', 'sum', 'count', 'revenue', 'sales']
            ...
            # Method 4: Default numeric column
            if not target_column and any(op in query_lower for op in ['max', 'min', 'average', 'sum', 'mean']):
                numeric_cols = df.select_dtypes(include=[np.number]).columns.tolist()
                if numeric_cols:
                    for col in numeric_cols:
                        if any(word in col.lower() for word in ['price', 'cost', 'amount', 'value', 'total']):
                            target_column = col
                            break
            ...
            if target_column and target_column in df.columns:
                ...
                if context['intent'] == 'counting' or 'count' in operations:
                    ...
                    # Update result and save immediately
                    statistical_result = {
                        **base_result,
                        'answer': answer,
                        'confidence': 1.0,
                    ...
                    # 🔥 IMMEDIATE SAVE AFTER SUCCESSFUL PROCESSING
                    _save_conversation_immediately(query, statistical_result)
                ...
                # For other operations, need numeric column
                    ...
                    _save_conversation_immediately(query, error_result)
                    return error_result
    ...
    if context['confidence'] > 0.7 and context['intent'] in ['analyze', 'summarize', 'categorical_analysis']:
        try:
            ...
            if specialist_result.get('confidence', 0) > 0.6:
                ...
                # 🔥 IMMEDIATE SAVE AFTER SUCCESSFUL PROCESSING
                _save_conversation_immediately(query, tabular_result)
                return tabular_result
                
        except Exception as e:
            logger.warning(f"Tabular specialist failed: {e}")
    
    # 📊 VISUALIZATION SPECIALIST (Third Chair)
        ...
        try:
            if has_statistical_operation and has_grouping:
                ...
                if viz_result:
                    ...
                    # 🔥 IMMEDIATE SAVE AFTER SUCCESSFUL PROCESSING
                    _save_conversation_immediately(query, viz_complex_result)
                    return viz_complex_result
            
            elif context['intent'] == 'visualize' or not has_statistical_operation:
                # Pure visualization logic (keeping your existing code)
                viz_filters_applied = []
                viz_working_df = df.copy()
                
                # Your existing filter patterns...
                filter_patterns = [
                    (r'(\w+)\s+(?:is\s+)?(?:less\s+than|<|below)\s+(\d+(?:\.\d+)?)', 'less_than'),
                    (r'(\w+)\s+(?:is\s+)?(?:greater\s+than|>|above)\s+(\d+(?:\.\d+)?)', 'greater_than'),
                    (r'(\w+)\s+(?:starting\s+with|begins?\s+with)\s+(?:letter\s+)?([a-zA-Z])', 'starts_with'),
                ...
                if fig:
                    ...
                    # 🔥 IMMEDIATE SAVE AFTER SUCCESSFUL PROCESSING
                    _save_conversation_immediately(query, viz_simple_result)
                    return viz_simple_result
    ...
    if context['intent'] == 'predict' or any(word in query.lower() for word in ['predict', 'forecast', 'model', 'estimate']):
        try:
            ...
            if target_column and target_column in df.columns:
                ...
                if model_result and 'error' not in str(model_result):
                    ...
                    if 'ai_insights' in model_result:
                        prediction_result['ai_insights'] = model_result['ai_insights']
                    
                    # 🔥 IMMEDIATE SAVE AFTER SUCCESSFUL PROCESSING
                    _save_conversation_immediately(query, prediction_result)
                    ...
                    # 🔥 SAVE EVEN ERROR RESULTS
                    _save_conversation_immediately(query, pred_error_result)
                    return pred_error_result
            else:
                pred_no_target_result = {
                    **base_result,
                ...
                # 🔥 SAVE EVEN ERROR RESULTS
                _save_conversation_immediately(query, pred_no_target_result)
                return pred_no_target_result
                
        except Exception as e:
            logger.error(f"Prediction specialist failed: {e}")
            pred_exception_result = {
                **base_result,
            ...
            # 🔥 SAVE EVEN ERROR RESULTS
            _save_conversation_immediately(query, pred_exception_result)
            return pred_exception_result
    ...
    # If we performed analysis but didn't return early, save now
    if analysis_performed and base_result.get('answer'):
        # 🔥 IMMEDIATE SAVE FOR COMPLEX ANALYSIS
        _save_conversation_immediately(query, base_result)
        return base_result

    # === GUARANTEED CONVERSATION SAVE FOR ALL REMAINING CASES ===
    # This ensures even fallback and error cases are saved
    if base_result.get('answer'):
        # 🔥 FINAL GUARANTEED SAVE
        _save_conversation_immediately(query, base_result)
    else:
        # Even if no answer, log the attempt
        base_result['answer'] = "❌ No response generated"
        base_result['confidence'] = 0.1
        base_result['method'] = 'no_response_error'
        _save_conversation_immediately(query, base_result)
...
#             # Method 2: If no numeric column found in entities, search for column names in query
#             if not target_column:
#                 # Look for column names in query (excluding filter columns already processed)
#                 filter_cols = [f.split()[0] for f in filters_applied] if filters_applied else []
                
#                 # First, try exact matches with numeric columns
#                 for col in df.columns:
#                     if (col.lower() in query_lower and 
#                         col not in filter_cols and 
#                         pd.api.types.is_numeric_dtype(df[col])):
...
#                 # If no exact match, try partial matches with numeric columns
#                 if not target_column:
#                     for col in df.columns:
#                         if (any(part in col.lower() for part in query_lower.split()) and 
#                             col not in filter_cols and 
...
#             # Method 3: For common statistical queries, use smart defaults
#             if not target_column:
#                 # Look for common statistical column patterns
#                 statistical_keywords = ['price', 'cost', 'amount', 'value', 'total', 'sum', 'count', 'revenue', 'sales']
#                 for keyword in statistical_keywords:
#                     if keyword in query_lower:
#                         for col in df.columns:
...
#             # Method 4: If still no target found and query mentions statistical operation, 
#             # find the first numeric column that makes sense
#             if not target_column and any(op in query_lower for op in ['max', 'min', 'average', 'sum', 'mean']):
#                 numeric_cols = df.select_dtypes(include=[np.number]).columns.tolist()
#                 if numeric_cols:
#                     # Prefer columns mentioned in query or with statistical-sounding names
#                     for col in numeric_cols:
#                         if any(word in col.lower() for word in ['price', 'cost', 'amount', 'value', 'total']):
#                             target_column = col
#                             break
                    
#                     # If no preference match, use first numeric column
#                     if not target_column:
#                         target_column = numeric_cols[0]
...
#     # === STEP 4: FALLBACK TO TABULAR AGENT === #########Modified on 20-08-2025##########
#     if not analysis_performed:
#         try:
#             tabular_agent = AgenticTabularProcessor(AI_MODELS['ollama'], ollama_model)
#             fallback_result = tabular_agent.process_tabular_query(df, query, context)
...
#     # === SINGLE CONVERSATION SAVE SECTION (FIXED) ===
#     try:
...
#         logger.info(f"💾 SAVE CHECK - Auth: {is_authenticated}, User: {username}, ConvMgr: {conv_manager is not None}")
        
#         if is_authenticated and username and conv_manager and 'answer' in base_result:
            
#             # Double-check username is still valid (prevent None username saves)
#             if not username or username.strip() == "":
#                 logger.error("❌ Username is empty or None, skipping save")
#             else:
#                 logger.info(f"💾 ATTEMPTING SAVE for user: {username}")
                
#                 # Prepare safe data for saving
#                 safe_query = str(query)[:500] if query else "Unknown query"
#                 safe_answer = str(base_result['answer'])[:1000] if base_result.get('answer') else "No answer"
#                 safe_confidence = float(base_result.get('confidence', 0.0))
#                 safe_file_type = str(st.session_state.get('file_type', 'unknown'))
...
#                 else:
#                     logger.error("❌ Conversation save returned False")
                
#                 # Save usage tracking (separate try block)
#                 try:
#                     usage_tracker = st.session_state.get('usage_tracker')
#                     if usage_tracker:
#                         usage_tracker.log_query(
...
#                             file_type=safe_file_type
#                         )
#                         logger.info("✅ Usage tracking saved")
#                     else:
#                         logger.warning("⚠️ Usage tracker not available")
#                 except Exception as usage_error:
#                     logger.error(f"❌ Usage tracking failed: {usage_error}")
#         else:
#             # Log why save was skipped
#             skip_reasons = []
#             if not is_authenticated:
#                 skip_reasons.append("not authenticated")
#             if not username:
#                 skip_reasons.append("no username")
#             if not conv_manager:
#                 skip_reasons.append("no conv_manager")
#             if 'answer' not in base_result:
#                 skip_reasons.append("no answer in result")
            
#             logger.info(f"💾 SAVE SKIPPED: {', '.join(skip_reasons)}")
            
#     except Exception as e:
#         logger.error(f"❌ CRITICAL: Conversation save failed with exception: {e}")
...
def display_model_results(model_result, show_feature_importance=True, 
                         show_predictions_plot=True, show_residuals=True):
...
################## New AuthManegemnt UI functions for User Authentication and Management. Added on 20-08-2025 ##################
...
def display_authentication():
    ...
    # Convert your logo to base64 (prefer PNG, fallback ICO)
    try:
        base64_logo = get_base64_of_image("logo.png")   # Recommended format
        logo_ext = "png"
    except FileNotFoundError:
        base64_logo = get_base64_of_image("logo.ico")   # Fallback
        logo_ext = "x-icon"
    ...
    <div class="auth-header">
        <div style="display: flex; align-items: center; justify-content: center; gap: 10px;">
            <img src="data:image/{logo_ext};base64,{base64_logo}" class="auth-logo" alt="Logo" style="width: 50px; height: 50px;">
            <h1 style="margin: 0; color:white; padding-bottom: 30px;">AI Data NooB</h1>
        </div>
        <p style="font-size: 0.9rem; margin-top: 0.5rem;padding-top: 5px; color: #e0e0e0;">
            Powered by Ascendum
        </p>
    </div>
    """, unsafe_allow_html=True)
...
#     <div class="auth-header">
#         <h1>🤖 AI Data NooB </h1>
#                 <p style="font-size: 0.9rem; margin-top: 0.5rem; color: #e0e0e0;">
#             Powered by Ascendum
#         </p>
...
def display_data_analysis_page(df, quality_report):
    ...
    with tab1:
        ...
        #  # Process query (either typed or from suggestion)
        # --- Smart Suggestions ABOVE query input ---
        if suggestions:
            st.markdown("**💡 Smart Suggestions:**")
            
            # Arrange suggestions in rows of 2
            for i in range(0, len(suggestions[:3]), 2):
                cols = st.columns(2)
                for j, col in enumerate(cols):
                    if i + j < len(suggestions[:3]):
                        suggestion = suggestions[i + j]
                        button_key = f"main_suggest_{i+j}_{hash(suggestion)}"
                        with col:
                            if st.button(suggestion, key=button_key, use_container_width=True):
                                st.session_state.selected_query = suggestion

        # --- Query Input BELOW smart suggestions ---
        query = st.text_input(
            "Ask about your data:",
        ...
        current_query = query or st.session_state.get('selected_query', '')
        ...
        if current_query:
            ...
            if st.session_state.last_processed_query != current_query:
                ...
                with st.spinner("🔄 Analyzing your request..."):
                    ...
                    # Process query
                    results = process_enhanced_query(df, current_query)
            ...
            # Display results (this part runs every time but doesn't duplicate saves)
            if 'answer' in results:
                ...
                # ✅ SHOW FEEDBACK SYSTEM (this won't duplicate conversations)
                display_feedback_system(current_query, results)
...
################################## Modified Welcome Page at 20-08-2025 ##################################
...
#         first_ai = (
#             f"👋 Welcome, {username}! "
#             "Please upload or integrate data to start chatting. "
#             "Meanwhile, you can ask general questions here."
#         )
...
def reset_welcome_message():
    ...
    # # Small status hint
    # if not AI_MODELS.get('ollama_available', False):
    #     st.info("ℹ️ Tip: Ollama not detected by your health check — "
    #             "ensure the Ollama daemon is running and the `llama3.2` model is pulled.")
...
def display_word_explorer(word_content):
   """Display Word document explorer - Mirror of PDF explorer"""
           ...
           for i, chunk in enumerate(display_chunks):
               with st.container():
                   ...
                   with col1:
                       st.markdown(f"**Chunk {chunk['id'] + 1}**")
                   with col2:
                       st.caption(f"Section: {chunk.get('section', 'N/A')} • {chunk['word_count']} words")
                   
                   st.text_area(
                       "Content",
                       chunk['content'],
                       height=120,
                       key=f"word_chunk_display_{chunk['id']}_{i}",
                       disabled=True
                   )
                   st.markdown("---")
       else:
       ...
       if tables:
           for i, table in enumerate(tables):
               ...
               st.download_button(
                   f"📥 Download Table {i+1} as CSV",
                   csv,
                   f"word_table_{i+1}.csv",
                   "text/csv",
                   key=f"download_word_table_{i}"
               )
               st.markdown("---")
       else:
           st.info("No tables found in the document")
   ...
   with tab3:
       st.markdown("#### Images")
       images = word_content.get('images', [])
       ...
       if images:
           for i, img in enumerate(images):
               ...
               with col1:
                   ...
                   st.write(f"📊 Index: {img['index']}")
               
               with col2:
                   st.info("📷 Embedded image detected (OCR not implemented for Word documents)")
...
def main():
    ...
    st.markdown("""
        <style>
            .main-header {
                background: linear-gradient(90deg, #616161 0%, #212121 100%);
    ...
    st.markdown(f"""
        <div class="main-header">
            <div class="main-header-content">
                <img src="data:image/{logo_ext};base64,{base64_logo}" class="main-logo" alt="Logo">
                <h1 style="color: white; margin: 0; padding-bottom: 1.5rem;">AI Data NooB</h1>
            </div>
    ...
    with st.sidebar:
        ...
        if AI_MODELS['ollama_available']:
            st.success(f"✅ SLM: {AI_MODELS.get('SLM_model', 'Connected')}")
        else:
        ...
        # elif st.session_state.current_word_content is not None:
        #     st.subheader("📄 Word Info")
        #     metadata = st.session_state.current_word_content.get('metadata', {})
        #     st.metric("Text Chunks", metadata.get('total_chunks', 0))
        #     st.metric("Tables Found", metadata.get('total_tables', 0))
        #     st.metric("Images", metadata.get('total_images', 0))
    ...
    if data_source == "Upload File":
        ...
        # Use file uploader WITHOUT type restrictions
        uploaded_file = st.file_uploader(
            "Choose your file",
            help=f"Supported: {', '.join(available_formats)} | Max size: 200MB"
            # ✅ NO 'type' parameter - this allows all file types
        )
                ...
                try:
                    if file_extension == "pdf" and PDF_PROCESSING_AVAILABLE:
                        ...
                        if success and isinstance(st.session_state.current_pdf_content, dict):
                            ...
                            # Track the upload (MOVED INSIDE the success block)
                        ...
                        success = handle_word_file_upload(uploaded_file)
                        if success and isinstance(st.session_state.current_word_content, dict):
                            doc = st.session_state.current_word_content
                            st.session_state['document_json'] = doc.get('document_json')
                        ...
                        if df is not None:
                            ...
                            # Track the upload (ALREADY CORRECTLY PLACED)
                            if st.session_state.get('authenticated') and st.session_state.get('username'):
                                track_file_upload(
                                    username=st.session_state.username,
                                    file_name=uploaded_file.name,
                                    file_type=f"tabular_{file_extension}",
                                    file_size_mb=file_size_mb
                                )
                        ...
                        st.info(f"Please upload one of: {', '.join(available_formats)}")
                    ...
                    with st.expander("🔧 Debug Information"):
                        ...
                        st.write(f"Extension detected: {file_extension}")
                        st.write(f"WORD_PROCESSING_AVAILABLE: {WORD_PROCESSING_AVAILABLE}")
    ...
    # --- Route Page Based on State --- ########## word Modified on 18-08-2025 ##########
    if (st.session_state.current_df is None and st.session_state.current_pdf_content is None and 
        st.session_state.current_word_content is None):
