...
def apply_scanning_effect(image):
    ...
    for y in range(0, height, 10):
        ...
        image_placeholder.image(scanning_overlay, caption='Scanning Image...', use_column_width=True)
        time.sleep(0.05)
    
    # Remove the scanning line after animation
    image_placeholder.image(image, caption='Image for Detection', use_column_width=True)
...
def predict_soil_nutrition(image, models):
    ...
    predicted_soil_type_encoded = classifier_model.predict([dominant_color])[0]
    predicted_soil_type = label_encoder.inverse_transform([predicted_soil_type_encoded])[0]
    return predicted_pH, predicted_soil_type
...
# Home page content
def home_page():
    st.title('Grow with Confidence: Introducing AI Assistant for Plant and Soil ')
    

    st.markdown("""
    Ever looked at your garden and wondered, "Is my soil healthy?" or "What's that spot on my tomato plant?"  Well, wonder no more! We're excited to introduce you to a revolutionary tool that empowers you to become an expert in your own backyard.
    """)
    
    st.image(".\\Pictures\\Home page\\Gemini_Generated_Image_h7i8v7h7i8v7h7i8.jpeg", use_column_width=True)
    st.markdown("""
    Our user-friendly application takes the guesswork out of plant care, providing you with valuable insights into your soil's health and potential plant problems. With just a few clicks, you can gain the knowledge you need to nurture your plants and ensure a bountiful harvest.
    """)
    st.markdown("""
    Here's how our app helps you cultivate a thriving garden:
    """)
    st.subheader("1.Unveiling the Secrets of Your Soil:")
    
    st.markdown("""
    Soil pH Analysis: Have you ever struggled to understand those cryptic soil test kits? Our app simplifies the process! Simply upload a picture of your soil, and our advanced image processing technology will analyze its color to determine its pH level. Knowing your soil's pH is crucial, as it directly affects the nutrients available to your plants. With this information, you can make informed decisions about amending your soil to create the perfect environment for your plants to flourish.
    """)

    st.subheader("2.Identifying Plant Diseases:")
    st.markdown("""
    Early Detection is Key: Spotting plant diseases early is vital for saving your precious crops. Our app acts as your personal plant doctor, using image recognition to identify various diseases affecting a wide range of plants, including tomatoes, peppers, and apple trees. With a clear diagnosis, you can take swift action to treat the problem and protect your harvest.
    """)

    st.subheader("3.Recognizing Cotton Field Threats:")
    st.markdown("""
    Protecting Your Cotton Crop: Cotton farmers face unique challenges when it comes to protecting their crops from pests. Our app utilizes image recognition technology to identify common cotton pests like aphids and bollworms. By equipping you with this knowledge, you can take appropriate measures to control pest populations and ensure a healthy, high-yielding cotton harvest.
    """)

    st.subheader("4.Ensuring Perfectly Ripened Fruit:")
    st.markdown("""
   Harvesting at the Peak of Flavor: Knowing exactly when to pick your fruit can be tricky. Our app takes the guesswork out of fruit ripening, specifically focusing on bananas,tomatoes and mangoes. Using advanced image recognition, the app can analyze a picture of your banana and determine its ripeness. This ensures you can enjoy your fruits at their peak of sweetness and flavor.
    """)

    st.subheader("Developed with Cutting-Edge Technology:")

    st.markdown("""
                Our app leverages the power of artificial intelligence and machine learning to deliver these incredible features.  We've trained our models on massive datasets of images, allowing them to accurately analyze photos and provide you with reliable results.
    """)
    st.markdown("""
...
def soil_nutrition_page():
    ...
    st.markdown("""
    Having healthy soil is the foundation of a thriving garden. But understanding complex soil test kits can be frustrating. Our app simplifies the process!
    """)
    st.markdown("""
    ...
    if uploaded_file is not None:
        ...
        predicted_pH, predicted_soil_type = predict_soil_nutrition(image, model)
        
        col1, col2 = st.columns(2)
        with col1:
            apply_scanning_effect(image)
        with col2:
            st.write(f"Predicted pH: {predicted_pH}\n\nPredicted Soil Type: {predicted_soil_type}\n\nDistribution: {crops[predicted_soil_type]['Distribution']}\n\nCharacteristics: {crops[predicted_soil_type]['Characteristics']}\n\nSuggested crops: {', '.join(crops[predicted_soil_type]['Suitable Crops'])}\n\nChemical Properties: {crops[predicted_soil_type]['Chemical Properties']}\n\nColor and Texture:{crops[predicted_soil_type]['Color and Texture']}")
    elif image:
        processor, model = load_model("Soil Nutrition")
        predicted_pH, predicted_soil_type = predict_soil_nutrition(image, model)
        col1, col2 = st.columns(2)
...
def plant_disease_page():
    ...
    if uploaded_file is not None:
        ...
        result = predict_other(image, processor, model, labels)
        ...
        with col2:
            ...
            output = f"{result}\n\n" + "\n\n".join([f"{key}: {value}" for key, value in disease_data.items()])
            st.write(output)
    elif image:
        processor, model = load_model("Plant disease")
        labels = labels_dict["Plant disease"]
...
def cotton_pests_page():
    ...
    if uploaded_file is not None:
        ...
        with col2:
            ...
            output = f"{result}\n\n" + "\n\n".join([f"{key}: {value}" for key, value in pest_data.items()])
            st.write(output)
    elif image:
        processor, model = load_model("Cotton Pests")
        labels = labels_dict["Cotton Pests"]
...
""")
    ...
    if image:
        ...
        with col1:
            ...
            for y in range(0, height, 10):
                ...
                draw = ImageDraw.Draw(scanning_overlay)
                    ...
                    # Draw the neon green scanning line with glow
                draw.line([(0, y), (width, y)], fill="lime", width=5)
                scanning_overlay = scanning_overlay.convert("RGB")
                enhancer = ImageEnhance.Brightness(scanning_overlay)
                scanning_overlay = enhancer.enhance(1.2)
        ...
        result_image = predict_yolo(image, model, conf_threshold, iou_threshold)
            
        with col2:
            st.image(result_image, caption='YOLO Result', use_column_width=True)
