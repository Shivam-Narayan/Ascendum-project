...
def login_user(request):
    ...
    if serializer.is_valid():
        ...
        # Load user JSON session history
        # user_history = load_user_history(email)  # Returns dict with 'sessions' list

        # Fetch user history from MongoDB by user id or email
        # user_history_doc = history_collection.find_one({"user_id": str(user.id)})
        # # user_history = user_history_doc.get('user_history', []) if user_history_doc else []
        # user_history = user_history_doc.get('chat_history', []) if user_history_doc else []

        # Fetch all chat history documents for this user
        user_history_cursor = history_collection.find({"user_id": str(user.id)})

        # Combine all chat_history arrays into one list
        user_history = []
        for doc in user_history_cursor:
            user_history.extend(doc.get('chat_history', []))
...
#         # Load user JSON session history
#         user_history = load_user_history(str(user.id))  # Returns dict with 'sessions' list
...
def change_password(request):
    ...
    # You can add more password validation as required here
...
#     if not pdf_session or pdf_session.get('user_id') != request.user.id:
#         return Response({'error': 'PDF session not found or access denied'}, status=status.HTTP_404_NOT_FOUND)
...
#     # Update JSON file chat history
#     user_email = request.user.email
#     user_history = load_user_history(user_email)
#     for sess in user_history['sessions']:
#         if sess['type'] == 'pdf' and sess['id'] == pdf_id:
#             sess['chat_history'] = pdf_session['chat_history']
#             break
#     save_user_history(user_email, user_history)
...
def save_chat_history_to_mongo(user_id, file_id, question, answer, timestamp=None):
...
#     from .chatbot_curerent_v3 import AgenticTabularProcessor, AI_MODELS
...
#     try:
#         sample_rows = raw_sample_data(df, 50)
#     except Exception as e:
#         sample_rows = []
...
def upload_excel_csv(request):
    ...
    # Remove in-memory session storage for dataset_id if exists
    DATASET_SESSIONS.pop(dataset_id, None)
...
# version 1 of ask_data_question
...
#     if not dataset_session or dataset_session.get('user_id') != request.user.id:
#         return Response({'error': 'Dataset session not found or access denied'}, status=status.HTTP_404_NOT_FOUND)
...
#     user_history = load_user_history(user_email)
#     for sess in user_history['sessions']:
#         if sess['type'] == 'excel' and sess['id'] == dataset_id:
#             sess['chat_history'] = dataset_session['chat_history']
#             break
#     save_user_history(user_email, user_history)
...
def ask_data_question(request, dataset_id):
    ...
    # Fetch UploadedFile record to verify ownership and get file path
    uploaded_file = get_object_or_404(
        UploadedFile,
        file_id=dataset_id,
        user=request.user
    ...
    timestamp = (datetime.utcnow() + timedelta(hours=5, minutes=30)).isoformat() + 'Z'
    save_chat_history_to_mongo(user_id=str(request.user.id), file_id=dataset_id,
                               question=question, answer=answer_data.get('answer'),
                               timestamp=timestamp)
