...
class UserManager(BaseUserManager):
    ...
    def create_admin(self, emp_id, email, password=None, **extra_fields):
        ...
        extra_fields.setdefault('is_staff', True)  # For Django admin access, if needed
...
class User(AbstractBaseUser):  # Removed PermissionsMixin
    ...
    is_admin = models.BooleanField(default=False)  # Custom admin field
...
class Dashboard(models.Model):
    ...
    password = models.CharField(max_length=255)  # if needed
    created_at = models.DateTimeField(default=timezone.now)
...
class DetectionResult(models.Model):
    dashboard_user = models.ForeignKey('Dashboard', on_delete=models.CASCADE)
    emp_id = models.CharField(max_length=50, default='UNKNOWN')  # or '' if you prefer
    image_id = models.ForeignKey('DashboardImage', on_delete=models.CASCADE)
    defect_result = models.JSONField(default=dict)  # default to empty JSON object
    created_at = models.DateTimeField(default=timezone.now)
